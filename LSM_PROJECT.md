...
Signal-to-check: after the two-stage detrend, defect residuals are still visible against the background. Stage D has re-run the gates and the 6-arm ablation ladder for the scalar rig (both done — see "Rig-v2 measured results" near the end of this document) — but neither reports a direct nT-contrast number the way the pre-Rig-v2 vector-rig measurement did; the ablation ladder reports recall/false-dig/localisation instead, so a like-for-like contrast comparison to the number below has not been produced. The pre-Rig-v2 vector-rig numbers (25.05 nT residual, 3.02–3.21× contrast) remain a historical baseline, not current. Background removal is still the whole point of the project, and interference still makes naive thresholding fail.
...
- **Sensor Configuration & Gradiometry (Rig-v2, corrected after a developer interview):** the real rig carries **three scalar total-field magnetometers** (middle + two, 50 cm apart) — each head outputs only `|B|`, never x/y/z, so what is actually measured is the *total-field anomaly*, `|B0+dB| − |B0| ≈ dB·B̂0`: only the projection of a defect's field onto the ambient field direction is visible. Three heads give both a **first difference** (`g1`, common-mode background rejection — the old two-head story) and a **second difference** (`g2 = b_hi + b_lo − 2·b_mid`, which additionally cancels a *linear* background gradient — the specific extra value the third head buys over a two-head gradiometer). The head-to-head amplitude ratio also inverts via 1/r³ for a genuine per-row measured stand-off. `|B|` is rotation-invariant, so rod sway/tilt moves head positions but never corrupts a reading by itself — almost certainly why the real instrument reports scalar in the first place. Whether/how much this configuration outperforms the pre-Rig-v2 vector-head model (which measured 3.21× contrast for single-head detrending vs. 1.45× for a two-head vertical gradiometer, `docs/interview-reference.md` §7) is not directly answered by a matching contrast number — Stage D's 6-arm ablation ladder measures recall/false-dig/localisation instead (see "Rig-v2 measured results" below) — but that older comparison is a historical baseline now either way, not the current default model's result.
