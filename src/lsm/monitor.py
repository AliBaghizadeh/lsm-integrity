...
def monitor_survey(conn, survey_id: str, cfg: Config) -> dict:
    ...
    pipeline_version, anomaly_version, _severity_version, _classify_version = (
    ...
    feature_drift = pd.DataFrame(
        columns=["feature", "psi", "ks_stat", "ks_pvalue", "status"]
    )
    ...
    feature_drift = feature_drift_report(
        anomaly_bundle.get("training_feature_summary", {}),
        features,
        feature_cols,
        monitor_cfg["psi_warn"],
        monitor_cfg["psi_block"],
    )
    ...
    prediction_drift = prediction_drift_report(
        ...
        indications["p_defect_cal"].to_numpy()
        if len(indications)
        else pd.Series([], dtype=float).to_numpy(),
        current_indications_per_km,
        monitor_cfg["indications_per_km_ratio_warn"],
    )
    ...
    if len(medians) >= 2 and not cur_reading.empty:
        ...
        background_regime = background_regime_shift(
            cur_median, hist, monitor_cfg["background_regime_z_threshold"]
        )
    ...
    if pipeline_version is not None:
        ...
        coverage = recompute_coverage_from_verifications(
            conn, pipeline_version, nominal
        )
...
def monitor_report_to_text(report: dict) -> str:
    ...
    blocked_features = [
        r["feature"] for r in report["feature_drift"] if r["status"] == "block"
    ]
    warned_features = [
        r["feature"] for r in report["feature_drift"] if r["status"] == "warn"
    ]
    lines.append(
    ...
    lines.append(
        f"Background regime: {br['n_bad']} axis/axes exceeded z-threshold ({br['z_scores']})"
    )
    cov = report["coverage"]
    if cov.get("n", 0) > 0:
        lines.append(
