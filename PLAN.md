...
| 3 — Detection (MAD vs IsolationForest) | Done. Pre-Rig-v2: recall gap closed to -0.006 after the EDA fix, confirmed at 800× scale (-0.029 [-0.033,-0.026], still a real small negative effect). Rig-v2 (2026-08-06): -0.006 [-0.061, 0.056], numerically unchanged. **Re-measured again post interference/defect physics refinement (2026-08-09): 0.000 [-0.050, 0.061] — MAD and IsolationForest now tie on the point estimate. Gate still does not pass the 0.15 margin, all three times.** **Confirmed three more times at 18M-row scale (2026-08-10): −0.007 / −0.013 / −0.014 under block CV / whole-line / temporal holdout — the two *harder* splits push it slightly further negative, not toward the gate. Seven measurements, two rigs, three grouping schemes; this is not a tuning problem.** **And now diagnosed rather than only confirmed: in the clean-room counterfactual (no interference, no welds) the same model reaches +0.199 [0.139, 0.255] — so the failure is caused by the confounders, not by the scalar rig's sensing physics. See the clean-room row below.** |
| 4 — Severity (LightGBM CQR + conformal) | Done pre-Rig-v2 (gate PASSED, coverage 0.920). Rig-v2: coverage 0.689 — gate FAILED, but still beat its own baseline (0.589). **Re-measured post physics refinement (2026-08-09): coverage 0.618 vs a baseline of 0.667 — the model now UNDERPERFORMS its own baseline, a further real regression, not smoothed over.** **Largely recovered at 18M-row scale (2026-08-10): 0.618 → 0.872, from more calibration data alone with no modelling change — this was a DATA-limited failure, not a broken conformal layer. But it passes only under block CV, the grouping the gate is configured against, and lands just below the band under whole-line (0.845) and temporal (0.860) holdout. Honest statement: "moved from badly broken to marginal," not "fixed."** |
| 4.5 — Demo app | Built, headlessly verified, and iterated through several real-use feedback rounds (last: 2026-08-07/08 — Rig-v2 row-count/GPS-dropout fixes; rebaked again 2026-08-09 for the physics refinement, see below). **2026-08-10: added the two signal plots to the "How it works" tab** in a "Why this is hard — the signal, in pieces" expander (`img/signal_components.png`, `img/signal_decomposition.png`, promoted from `docs/img/` so they reach the public snapshot too). They make the case the seven-stage pipeline exists for — welds are the loudest thing in the data and defects the quietest — which nothing in the app previously showed. Rendered from committed PNGs, not plotted live: they need the generator's own per-source traces, which a served survey doesn't carry, and both `st.image` calls are `.exists()`-guarded so a missing asset degrades rather than crashes. **No rebake or re-measure was needed for the clean-room work** — `weld.enabled` defaults to `true`, so the shipped corpus/models are byte-identical, and the app hardcodes no metrics (it renders the auto-generated `serving/model_card.md`). HF Spaces deploy still deferred, see next-steps. |
| 5 — Classification and risk ranking | Done (2026-07-30). Demoted to a synthetic-only capability demo after the ROSEN developer interview confirmed no real labelled defect-type data exists. Gate did not pass pre-Rig-v2 (SCC recall 0.125), under Rig-v2 (0.042), or post physics refinement (0.023) — expected, not new. **But a real secondary finding emerged 2026-08-09: making severity type-correlated gave the classifier actual non-zero recall on weld/dent/corrosion (0.258/0.144/0.171, up from ~0) — SCC alone stays unrecoverable, by design (its severity range overlaps weld's).** **A correction forced at 18M-row scale (2026-08-10): the pre-Rig-v2 vector rig's 0.999 interference precision — for a long time this project's strongest positive result — does NOT survive the rig change. Across all three grouping schemes the scalar-rig classifier sits at 0.265–0.298, and in two of the three it is at or below the trivial majority-class baseline. "Supervised classification solves interference discrimination" is a vector-rig claim only and must not be carried over. Unlike severity, this is not data-limited: 6× the data moves it nowhere.** |
| 6 — Scale rehearsal | Done (2026-07-31) pre-Rig-v2 at ~9.6M rows, and **re-run under Rig-v2 + the physics refinement on 2026-08-10** at 18.0M rows / 360 defects / 77 min / 49.4 GB peak RSS, under three grouping schemes instead of one. **Settles two open questions and forces one correction.** Severity coverage recovered 0.618 → 0.872 on more calibration data alone, no modelling change — but passes only under block CV (the gate's own grouping) and lands just below the band under whole-line (0.845) and temporal (0.860) holdout, so the honest claim is "moved from badly broken to marginal," not "fixed." Detection gap unmoved (−0.007 / −0.013 / −0.014, confirmations 5–7). **Correction: the pre-Rig-v2 0.999 interference precision does NOT survive the rig change** — 0.265–0.298 across all three splits, at or below the majority-class baseline in two of them; that claim is vector-rig-only from now on. Getting it to run at all required fixing 5 Rig-v2 staleness bugs in the no-CI-coverage rehearsal path, 2 of which would have failed *silently* with a plausible wrong report. See `docs/stage6-scale-rehearsal.md` + `docs/experiment-log.md`. |
...
| **Clean-room experiment** | **Done (2026-08-10).** `scripts/cleanroom_experiment.py`, new, plus a `weld.enabled` flag on `WeldConfig` (physical default `true`; `false` removes the weld train's *physics*, not just its label). Tests the proposal put to the developer team — run in-house test spools with known defects, isolated pipe, no interference and no construction features — before anyone spends money on it. Two experiments: E1, the within-domain ceiling (a diagnostic; removing confounders from a confounder-limited problem *has* to help, so it proves nothing on its own), and E2, a train-on-one-domain/test-on-the-other's-held-out-lines transfer matrix (the decision-relevant half). **E1 headline: the detection recall gap goes to +0.199 [0.139, 0.255] — the first measurement in this project where IsolationForest beats MAD at all** (every prior one, nine across two rigs, sat at or below zero), with localisation 812 cm → 75 cm (10.8×). It still fails the gate, narrowly (CI lower bound 0.139 vs the 0.15 bar). This turns the standing "Level 2" diagnosis of the detection failure into a measurement, and names *which* information the scale rehearsal's "information-limited" verdict was missing: the confounders, not the scalar rig's sensing physics. **E2 headline: it does not transfer.** Clean-room training makes the *field* model worse on severity MAE (15.8 → 19.4) and Brier (0.925 → 0.965) at both scales tested, and it cannot predict interference at all, having never seen the class. **Net: the proposal survives with its purpose changed — an in-house campaign is a diagnostic and ceiling-setting instrument, not a training corpus.** Full detail: `LSM_PROJECT.md`'s "The clean-room experiment" + `reports/cleanroom-transfer.md`. |
| **Signal decomposition plots** | Done (2026-08-09). `scripts/plot_signal_decomposition.py`, new: regenerates one survey in memory (reusing `generate.py`'s own building blocks, same pattern as `ablation_ladder.py`/`pod_by_angle.py`) and renders how the raw field is built from its physical sources and torn back down to isolate the true defect signal. Two outputs: `docs/img/signal_decomposition.png` (5-panel narrative, background→raw→residual→isolated) and `docs/img/signal_components.png` (each source — background/welds/interference/defects — on its own y-scale, `--components` flag). **A real bug was found and fixed while building the second view**: isolating each source by subtracting the background's scalar *median* left its smooth drift/wave bleeding into the interference/defects panels, since the underlying `mag_X_only` traces are `\|B_background + field_X\|`, not `field_X` alone — fixed by subtracting the full background array. Both images are candidates for the Medium article drafts (see below). |
...
312/312 tests pass, ruff + mypy clean. GitHub (`AliBaghizadeh/lsm-integrity`, private) is a
...
1. **A cloud-account decision** — still the single blocker on the rest of Stage 7 (S3 +
   ...
   infrastructure for.
2. **Explicitly-flagged not-yet-run items** (all stated honestly in `LSM_PROJECT.md`, not
   hidden). *Two of the three items previously listed here were closed on 2026-08-10 — the
   scale rehearsal under Rig-v2, and the severity-coverage question it answered; both are
   recorded in the Stage 6 and Stage 4 rows above.* What remains:
   ...
   - The clean-room experiment's three natural follow-ups, in value order: the **mixing
     curve** (train on lab + k% field data — the most decision-relevant, since it would say
     whether a little field data dominates a lot of lab data), the **data-efficiency curve**
     (how many test spools before the benefit saturates, i.e. what to actually budget), and
     the **defect-spacing sweep** (a real test facility packs defects far denser than 6/km;
     the experiment held density equal across domains to avoid confounding, which leaves the
     density question itself unmeasured).
   - Severity's recovery is marginal, not robust: it passes under block CV and fails under
     both harder splits. Whether that closes with more calibration data again, or is a real
     ceiling, is untested.
...
## Stage 6 — Scale rehearsal  *(built 2026-07-31 on the pre-Rig-v2 vector rig; re-run under Rig-v2 on 2026-08-10 at 18.0M rows / 360 defects, under three grouping schemes — see "Current status" above and `docs/stage6-scale-rehearsal.md`)* — the answer to "millions of rows"
