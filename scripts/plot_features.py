...
def bands(df: pd.DataFrame, flag_col: str) -> list[tuple[float, float]]:
    ...
    return [
        (chainage[a], chainage[min(b, len(df) - 1)])
...
def report(df: pd.DataFrame, cfg) -> None:
    ...
    print(
        f"  raw field |B|                   {np.linalg.norm(df[['bx_nt', 'by_nt', 'bz_nt']], axis=1).mean():10.0f} nT"
    )
    ...
    print(
        f"  contrast (defect median / background median)  {d['r_mag_nt'].median() / b['r_mag_nt'].median():6.1f}x"
    )
    print(
        f"  defect signal as a fraction of the raw field  "
    ...
    for col, unit in [
        ("r_mag_nt", "nT"),
        ("peak_prominence_nt", "nT"),
        ("fwhm_m", "m"),
        ("peak_asymmetry", ""),
        ("decay_exponent", ""),
    ]:
        print(
            f"  {col + ' (' + unit + ')':22s}{d[col].median():12.2f}{i[col].median():14.2f}"
        )
    print(
        f"  -> amplitude ratio {i['r_mag_nt'].median() / d['r_mag_nt'].median():.2f}x, "
    ...
    for ax in "xyz":
        ...
        print(
            f"  B{ax}: background std {raw_std:7.2f} nT -> head-difference std {diff_std:6.2f} nT "
    ...
    print(
        f"  Doubled sensor noise alone (sigma*sqrt(2)) would be {noise_floor:.2f} nT; the extra "
    ...
    print(
        "  this generator's vertical gradient is a real, non-zero term, not yet a fully independent"
    )
    print(
        "  per-head geomagnetic realisation. Treat the gradiometer numbers below as directionally"
    )
    print(
        "  honest, not as a claim about a specific real-world common-mode rejection figure."
    )
    ...
    print(
        f"  -> the gradiometer is {verdict} for detection here. Detrending has already removed the"
    )
    print(
        "     common mode, so differencing adds sqrt(2) noise and subtracts part of the near-field"
    )
    print(
        "     signal across a 0.5 m baseline. It earns its place where the background is NOT smooth"
    )
    print(
        "     along-track -- the assumption the detrend depends on -- and because it is instantaneous:"
    )
    print(
        "     no fitting window, so no edge rows and nothing to get wrong at a survey boundary."
    )
...
def plot(df: pd.DataFrame, survey_id: str, chainage_range) -> plt.Figure:
    ...
    panels = [
        ...
        (
            "r_mag_nt",
            "|residual| (nT)",
            "after robust detrend + high-pass: defects appear, so does interference",
        ),
        (
            "dr_ds_nt_per_m",
            "d|r|/ds (nT/m)",
            "along-track gradient -- one sensor, a derivative",
        ),
    ]
    if has_grad:
        panels.append(
            (
                "g_mag_nt_per_m",
                "|vertical gradient| (nT/m)",
                "two heads at 0.5 m: common mode cancelled without fitting anything",
            )
        )

    fig, axs = plt.subplots(
        len(panels), 1, figsize=(12, 2.3 * len(panels)), sharex=True
    )
    fig.suptitle(
