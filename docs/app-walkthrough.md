...
A second expander, **"Why this is hard — the signal, in pieces"** (added 2026-08-10), holds the
two signal plots. The first shows each physical source on its own y-scale over one 2 km line:
girth welds are the loudest thing in the data by a wide margin (~163 joints per line),
interference reaches over 150 nT, and the defects — the only thing anyone wants — peak around
11 nT. The second walks the same survey from raw field through two-stage detrending to the
isolated defect signal. Both render from committed PNGs (`img/signal_components.png`,
`img/signal_decomposition.png`, regenerated by `scripts/plot_signal_decomposition.py`), not
plotted live: they need the generator's own per-source traces, which a served survey does not
carry — which is itself the point being made.

**Say:** "Every scenario in this app runs this exact same seven-stage path — this tab is the
map of it." Then open the second expander: **"and this is why it needs seven stages — the
thing we're looking for is the smallest signal in the data, by an order of magnitude."**
